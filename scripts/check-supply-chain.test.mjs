...
test("requires exact canonical versions without ranges", () => {
  ...
  expectInvalid(
    (data) => { data.npm_overrides["brace-expansion"] = "^5.0.9"; },
...
for (const [dependency, mutation] of [
  ...
  ["brace-expansion range", (packageData) => { packageData.overrides["brace-expansion"] = "^5.0.9"; }],
...
test("surface validation rejects a lockfile not regenerated to the brace-expansion override", () => {
  withSurfaceFixture((root) => {
    ...
    assert.throws(
      ...
      /package-lock\.json brace-expansion must resolve to 5\.0\.9/,
    );
  });
});
